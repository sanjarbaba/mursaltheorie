...
import { SOURCES,SIGNS,signByCode,WORLDS,CHAPTERS,LEVELS,MODES,SCENARIOS } from './_game-data.js';
...
export async function GET(request) {
  ...
  try {
    ...
    if (!new URL(request.url).searchParams.has('check')) data.content = {SOURCES,SIGNS,signByCode,WORLDS,CHAPTERS,LEVELS,MODES,SCENARIOS};
    return json({data});
  } catch {
