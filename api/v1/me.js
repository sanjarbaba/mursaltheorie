...
async function exportAccount(sql, userId) {
  const [profiles, progress, examAttempts, entitlements, devices] = await Promise.all([
  ...
  return accountExport({ profile: profiles[0], progress, examAttempts, entitlements, devices });
}
